...
function builtInPrompt(scopeRoot: string): string {
  return [
    ...
    'root.',
    '',
    'If the question is a brief with several numbered items, answer every item in the same order,',
    'each under its own markdown heading that starts with the item\'s number, e.g. "## 1. <short title>".',
    'Put each item\'s citations inside its own section. If an item has nothing to report — the thing',
    'does not exist, or you could not find it within budget — say so explicitly under its heading',
